...
from .utils_large import MyQADataset, MyQAPromptDataset, MyDataLoader
...
class NLPFewshotGymSingleTaskData(object):
    ...
    def __init__(self, logger, args, data_path, data_type, is_training):
        ...
        self.task_name = "_".join(self.data_path.split("/")[-1].split("_")[:-1])
    ...
    def load_dataset(self, tokenizer, do_return=False):
        ...
        if self.load and os.path.exists(preprocessed_path):
            ...
            for dp in self.data:
                ...
                #for item in dp[1]:
                 #   print("item:",item)
                outputs.append([self.extra_id_0 + item  for item in dp[1]]) # is a list
        ...
        if self.args.do_prompt:
            self.dataset = MyQAPromptDataset(input_ids, attention_mask,
                                            ...
                                            is_training=self.is_training, prompt_num=self.args.prompt_num, model=self.args.model)
        else:
