...
class MyQAPromptDataset_AE(Dataset):
    def __init__(self,
                 input_ids, attention_mask,
                 decoder_input_ids, decoder_attention_mask,
                 task_prefix, task_names, all_task_prompts1,all_task_prompts2, ontology, in_metadata=None, out_metadata=None,
        ...
        self.all_task_prompts1 = all_task_prompts1
        self.all_task_prompts2 = all_task_prompts2
    ...
    def __getitem__(self, idx):
        ...
        if not self.is_training:
            idx = self.in_metadata[idx][0]
            return self.input_ids[idx], self.attention_mask[idx], random.choice(self.all_task_prompts1[self.task_prefix[idx]]), random.choice(self.all_task_prompts2[self.task_prefix[idx]]), ontology_type1, ontology_type2, ontology_tensor_general, self.task_prefix[idx]

        in_idx = np.random.choice(range(*self.in_metadata[idx]))
        out_idx = np.random.choice(range(*self.out_metadata[idx]))
        task_prompt1 = random.choice(self.all_task_prompts1[self.task_prefix[idx]])
        task_prompt2 = random.choice(self.all_task_prompts2[self.task_prefix[idx]])
        return self.input_ids[in_idx], self.attention_mask[in_idx], \
            self.decoder_input_ids[out_idx], self.decoder_attention_mask[out_idx], task_prompt1,task_prompt2, ontology_type1, ontology_type2, ontology_tensor_general
