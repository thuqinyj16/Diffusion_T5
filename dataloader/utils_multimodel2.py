...
class MyQAPromptDataset_AE(Dataset):
    def __init__(self,
                 ...
                 general_num=50, task2id=None,intr_emb=None):
    ...
    def __getitem__(self, idx):
        ...
        task_intr = self.intr_emb[self.task_names[idx]].weight
        ...
        if not self.is_training:
            idx = self.in_metadata[idx][0]
            return self.input_ids[idx], self.attention_mask[idx], task_intr, ontology_type1, ontology_type2, ontology_tensor_general, self.task_prefix[idx]

        in_idx = np.random.choice(range(*self.in_metadata[idx]))
        out_idx = np.random.choice(range(*self.out_metadata[idx]))
        task_prompt = random.choice(self.all_task_prompts[self.task_prefix[idx]])
        return self.input_ids[in_idx], self.attention_mask[in_idx], \
            self.decoder_input_ids[out_idx], self.decoder_attention_mask[out_idx], task_intr, ontology_type1, ontology_type2, ontology_tensor_general
