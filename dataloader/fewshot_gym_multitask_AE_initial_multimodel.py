...
from .utils_initial_multimodel import MyQADataset, MyQAPromptDataset, MyQAPromptDataset_AE, MyDataLoader
    ...
    def __init__(self, logger, args, data_path, tasks, data_split, data_type, is_training, is_test=False):
        ...
        for task in sorted(tasks):
            ...
            for prefix in prefixes:
                ...
                task_prompt = []
                prompt_weight_dir1 = os.path.join("/data/private/zhubiru/CrossFit_prompt3/models/few_shot_prompt", 'singletask-' + task, 'prompt_weight')
                load_flag = False
                for prompt_dir in os.listdir(prompt_weight_dir1):
                    '''
                    ...
                    if args.recover_multiple_seeds and 'best' in prompt_dir:
                        continue
                    task_prompt.append(torch.load(os.path.join(prompt_weight_dir1, prompt_dir)))
                    #print("task_prompt[0]:",task_prompt[0])
                    print("prompt_weight_dir:",os.path.join(prompt_weight_dir1,prompt_dir))
                    print("pretrained task promts!!!!!!!!!")
                    load_flag = True
                task_prompt2 = []
                prompt_weight_dir2 = os.path.join("/data/private/zhubiru/CrossFit_prompt3/models/few_shot_prompt_large", 'singletask-' + task, 'prompt_weight')
                load_flag = False
                for prompt_dir in os.listdir(prompt_weight_dir2):
                    '''
                    if '_' + str(args.select_prefix) + '_' not in prompt_dir:
                        continue
                    '''
                    ...
                    if args.recover_multiple_seeds and 'best' in prompt_dir:
                        continue
                    task_prompt2.append(torch.load(os.path.join(prompt_weight_dir2, prompt_dir)))
                    #print("task_prompt[0]:",task_prompt[0])
                    print("prompt_weight_dir:",os.path.join(prompt_weight_dir2,prompt_dir))
                    print("pretrained task promts!!!!!!!!!")
                    load_flag = True
                ...
                self.data.append({
                    ...
                    "task_prompt1": task_prompt,
                    "task_prompt2": task_prompt2,
    ...
    def load_dataset(self, tokenizer, do_return=False):
        ...
        if self.load and os.path.exists(preprocessed_path) and (not self.do_AE) and (not self.do_ensemble):
            ...
            for task in self.data:
                ...
                all_task_prompts1[task["task_prefix"]] = task["task_prompt1"]
                all_task_prompts2[task["task_prefix"]] = task["task_prompt2"]
        ...
        if self.args.do_prompt and not (self.args.do_AE or self.args.do_ensemble):
            ...
            self.dataset = MyQAPromptDataset_AE(input_ids, attention_mask,
                                            decoder_input_ids, decoder_attention_mask,
                                            task_prefix, task_names, all_task_prompts1, all_task_prompts2,ontology,
                                            in_metadata=None, out_metadata=metadata,
                                            is_training=self.is_training, prompt_num=self.args.prompt_num,
