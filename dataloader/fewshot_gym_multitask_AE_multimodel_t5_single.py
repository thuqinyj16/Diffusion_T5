...
from .utils_multimodel_t5 import MyQADataset, MyQAPromptDataset, MyQAPromptDataset_AE, MyDataLoader
...
class NLPFewshotGymMultiTaskData_AE(object):
        ...
        for task in sorted(tasks):
            ...
            for prefix in prefixes:
                ...
                for prompt_dir in os.listdir(prompt_weight_dir):
                    ...
                    #print("task_prompt[0]:",task_prompt[0])
                    print("prompt_weight_dir:",os.path.join(prompt_weight_dir,prompt_dir))
                    print("pretrained task promts!!!!!!!!!")
                    load_flag = True
                if load_flag:
                    c1 += 1
                else:
                    ...
                    c2 += 1
                    print("no pretrained task prompts!!!!!!!!!")
                self.data.append({
                    "task_name": task,
                    "task_prefix": prefix,
    ...
    def decode(self, tokens,name):
        if (name=="t5"):
            return self.tokenizer2.decode(tokens, skip_special_tokens=True, clean_up_tokenization_spaces=True)
        elif(name=="bart"):
            return self.tokenizer1.decode(tokens, skip_special_tokens=True, clean_up_tokenization_spaces=True)
    ...
    def load_dataset(self, tokenizer1,tokenizer2, do_return=False):
        self.tokenizer1 = tokenizer1
        self.tokenizer2 = tokenizer2
        postfix = tokenizer1.__class__.__name__.replace("zer", "zed")
        #postfix = tokenizer1.__class__.__name__.replace("zer", "zed")
        ...
        if self.load and os.path.exists(preprocessed_path) and (not self.do_AE) and (not self.do_ensemble):
            ...
            for task in self.data:
                ...
                if self.data_split == "train" or self.data_split == "all":
                    for dp in task["train_examples"]:
                        ...
                        print("dp[1][0]:!!!!",dp[1][0])
                        outputs2.append([self.extra_id_0 + item  for item in dp[1]])
            ...
            outputs2,metadata2 = self.flatten(outputs2)
            #print("metadata!!!!!!!:",metadata[0])
            #print("metadata2!!!!!!:",metadata2[0])
            #print("metadata!!!!!!!:",metadata[1])
            #print("metadata2!!!!!!:",metadata2[1])
            #print("outputs2[0]!!!!!:",outputs2[0])
            ...
            if self.args.do_lowercase:
                ...
                outputs2 = [output0.lower() for output0 in outputs2]
            ...
            tokenized_input = tokenizer1.batch_encode_plus(inputs,
                                                         pad_to_max_length=True,
                                                         max_length=self.args.max_input_length)
            self.logger.info("Tokenizing Output ...")
            tokenized_output = tokenizer1.batch_encode_plus(outputs,
                                                       pad_to_max_length=True,
                                                       max_length=self.args.max_output_length)
            tokenized_input2 = tokenizer2.batch_encode_plus(inputs,
                                                         pad_to_max_length=True,
                                                        # padding='max_length',
                                                         truncation=True,
                                                         #return_tensors="pt",
                                                         ...
                                                         max_length=self.args.max_input_length)
            self.logger.info("Tokenizing Output ...")
            tokenized_output2 = tokenizer2.batch_encode_plus(outputs2,
                                                         padding='max_length',
                                                         truncation=True,
                                                         #return_tensors="pt",
                                                        pad_to_max_length=True,
            ...
            input_ids2, attention_mask2 = tokenized_input2["input_ids"], tokenized_input2["attention_mask"]
            decoder_input_ids2, decoder_attention_mask2 = tokenized_output2["input_ids"], tokenized_output2["attention_mask"]
            print("decoder_input_ids[0]:",decoder_input_ids[0])
            print("decoder_input_ids2[0]:",decoder_input_ids2[0])
            print("decoder_input_ids[1]:",decoder_input_ids[1])
            print("decoder_input_ids2[1]:",decoder_input_ids2[1])
        ...
        if self.args.do_prompt and not (self.args.do_AE or self.args.do_ensemble):
            ...
            self.dataset = MyQAPromptDataset_AE(input_ids, attention_mask,
                                            decoder_input_ids, decoder_attention_mask,
                                            input_ids2,attention_mask2,decoder_input_ids2,decoder_attention_mask2,
                                            task_prefix, task_names, all_task_prompts, ontology,
                                            in_metadata=None, out_metadata=metadata2,
                                            is_training=self.is_training, prompt_num=self.args.prompt_num,
    ...
    def evaluate(self, predictions, task_names, verbose=False):
        ...
        #print("task2score:",task2score)
        #print("mean:",np.mean(list(task2score.values())))
        val_list = []
        for key in task2score.keys():
            #print("val:",task2score[key])
            #print("val2:",task2score[key].items[1])
            for k,v in task2score[key].items():
                #print("v:",v)
                val_list.append(v)

        return task2score, np.mean(val_list)
