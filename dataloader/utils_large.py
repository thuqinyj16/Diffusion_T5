...
class MyQAPromptDataset(Dataset):
    def __init__(self,
                 ...
                 decoder_input_ids, decoder_attention_mask,
                 in_metadata=None, out_metadata=None,
                 is_training=False, prompt_num=100, model='t5.1.1.lm100k.large'):
        self.prompt_prefix = [- (i + 1) for i in range(prompt_num)]
    ...
    def __getitem__(self, idx):
        if not self.is_training:
            idx = self.in_metadata[idx][0]
            out_idx = np.random.choice(range(*self.out_metadata[idx]))
            return self.input_ids[idx], self.attention_mask[idx], self.decoder_input_ids[out_idx].masked_fill_(self.decoder_input_ids[out_idx] == self.tokenizer.pad_token_id, -100), self.decoder_attention_mask[out_idx]

        in_idx = np.random.choice(range(*self.in_metadata[idx]))
        out_idx = np.random.choice(range(*self.out_metadata[idx]))

        return self.input_ids[in_idx], self.attention_mask[in_idx], \
            self.decoder_input_ids[out_idx].masked_fill_(self.decoder_input_ids[out_idx] == self.tokenizer.pad_token_id, -100), self.decoder_attention_mask[out_idx]
